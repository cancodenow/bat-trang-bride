...
import { preloadUIAssets, preloadLevelAssets, createModalFrame, createDevSkipButton , createBackButton } from "../UIHelpers";
...
export default class PorkRibSelectionScene extends Phaser.Scene {
  ...
  createSidebar() {
    ...
    this.add
      .text(sw / 2, 16, "Mua Sườn / Buy Ribs", {
        fontSize: "18px", color: "#bb9882",
        fontFamily: "SVN-Pequena Neo", fontStyle: "bold", align: "center",
      })
      .setOrigin(0.5, 0);
    ...
    this.add.text(15, 56, "Price per piece:", {
      fontSize: "15px", color: "#bb9882", fontFamily: "SVN-Pequena Neo",
    });
    this.add.text(sw - 15, 56, "32K VND", {
      fontSize: "15px", color: "#ffffff", fontFamily: "SVN-Pequena Neo",
    }).setOrigin(1, 0);

    this.add.text(15, 76, "Budget:", {
      fontSize: "15px", color: "#bb9882", fontFamily: "SVN-Pequena Neo",
    });
    this.add.text(sw - 15, 76, this.MONEY + "K VND", {
      fontSize: "15px", color: "#ffffff", fontFamily: "SVN-Pequena Neo",
    }).setOrigin(1, 0);

    this.add.rectangle(sw / 2, 100, sw - 30, 1, 0x3a5a7a);

    this.add.text(sw / 2, 108, "Selection", {
      fontSize: "15px", color: "#bb9882",
      fontFamily: "SVN-Pequena Neo", fontStyle: "bold",
    }).setOrigin(0.5, 0);

    this.selectedCountText = this.add.text(15, 128, "Pieces: 0 ", {
      fontSize: "17px", color: "#ffffff", fontFamily: "SVN-Pequena Neo",
    });
    ...
    this.add.text(sw / 2, 188, "one piece = 200g", {
      fontSize: "14px", color: "#bb9882",
      fontFamily: "SVN-Pequena Neo", align: "center",
    }).setOrigin(0.5, 0);

    this.statusText = this.add.text(sw / 2, 212, "Click the rib to add one piece.", {
      fontSize: "14px", color: "#bb9882",
      fontFamily: "SVN-Pequena Neo", align: "center",
  ...
  createRibDisplay() {
    ...
    img.on("pointerover", () => img.setDisplaySize(imgSize + 14, (imgSize + 14) * 0.85));
  ...
  updateSidebar() {
    ...
    if (count === 0) {
      ...
      this.statusText.setText("You have selected " + count*0.2 + " kg of pork ribs");
  ...
  checkSelection() {
    if (this.quantity < 5) {
      this.statusText.setText("That is not enough pork ribs. \nPlease choose more to make a full kilogram");
      this.statusText.setColor("#ff6666");
    } else {
  ...
  showBargainModal() {
    ...
    const { container } = createModalFrame(this, 640, 260, { overlayAlpha: 0.6 });
    const frameY = height / 2;  // frame is centered here by createModalFrame

    const msg = this.add
      .text(width / 2, frameY , "Let's keep within the budget, dear.\nDo you want to bargain?", {
        fontSize: "22px",
        color: "#ffffff",
    ...
    const yesBtn = this.add
      ...
      .setScale(YES_SCALE)
      .setInteractive({ useHandCursor: true })
      .on("pointerover", function () { this.setScale(YES_SCALE * 1.08); })
      .on("pointerout",  function () { this.setScale(YES_SCALE); })
    ...
    const momBtn = this.add
      ...
      .setScale(NO_SCALE)
      .setInteractive({ useHandCursor: true })
      .on("pointerover", function () { this.setScale(NO_SCALE * 1.08); })
      .on("pointerout",  function () { this.setScale(NO_SCALE); })
